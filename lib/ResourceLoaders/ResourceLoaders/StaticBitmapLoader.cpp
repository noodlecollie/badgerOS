...
namespace ResourceLoaders
{
	BadgerGL::ConstBitmapSurface loadStaticBitmap(const Resources::StaticImageMetadata& meta)
	{
		...
		BGRS_ASSERTD(meta.pixelFormat.totalBitDepth() > 0, "Resource bit depth was zero.");
		BGRS_ASSERTD(meta.pixelFormat.totalBitDepth() <= BadgerGL::BitmapSurface::MAX_BIT_DEPTH, "Resource bit depth exceeded max depth.");
		...
		return ConstBitmapSurface(ConstBitmapInitialiser(meta.width, meta.height, &meta.pixelFormat, meta.data));
	}
}
